...
**Fechamento do ciclo após a primeira entrega:** auditoria independente implementada; 11 testes novos; **385 testes passaram**, em 94,56 s, sobre o commit local `13184fb24ad44b5b94f2653d467e7ebba92881b4`, com Core 3.2.0 e árvore limpa. Ruff 0.16.5 e migração `--check` aprovados. A tentativa anterior de rodar com a árvore suja falhou em 21 testes pela guarda `DirtyWorkingTreeError`; não foi desativada. O commit local resolveu a condição de execução, sem mudar o Core.

O auditor do ZIP encontrou **69.024 linhas monetárias com unidade incompatível** nos quatro demonstrativos examinados e **80 combinações companhia/exercício com data da versão divergente**. São contagens do arquivo fonte inteiro, não de posições ou sinais elegíveis. O auditor retorna `NOT_READY`, exit code 2, como esperado; não é uma trava automática do comando de backtest. O caminho histórico de ingestão permanece preservado, portanto os defeitos de produção continuam pendentes de correção em dados derivados isolados.

O ledger v2 foi atualizado mecanicamente de 8 para 15 entradas no checkout de auditoria; isso não preenche metadados históricos desconhecidos nem corrige as janelas legadas. Relatórios H14–H16 foram recuperados sem regeneração. Banco operacional, ledger legado, config e atestado tiveram hashes conferidos antes/depois: **inalterados**. Nenhuma alteração foi enviada ao GitHub.

A cobertura foi efetivamente remensurada nesta sessão e reproduziu H17=56, H18=49,5 e H19=53 em 104 datas. Uma suspeita adicional foi rejeitada na amostra: os 455 grupos do FRE 2023 não mostraram mistura observável entre quantidade máxima futura e primeira data entre múltiplas linhas; isso não resolve a data-base de split nem certifica outros anos. A auditoria preserva tanto achados quanto checagens negativas.
