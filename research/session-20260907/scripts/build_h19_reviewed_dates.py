...
for row in queue:
    ...
    installment_dates=['2023-05-24','2023-07-26'] if (row['ticker']=='ALSO3'
        and row['ex_date']=='2023-05-02' and Decimal(row['value_per_share'])==Decimal('0.25950720796')) else []
    rows.append({**row, 'reviewed_payment': fact, 'payment_date_reviewed': fact is not None,
                 'candidate_payment_dates': sorted(set(row['candidate_payment_dates']) |
                    set(weak) | set(installment_dates) | ({fact['payment_date']} if fact else set())),
                 'reviewed_installment_schedule_dates':installment_dates,
...
result['installment_schedule_reviews']=[dict(ticker='ALSO3',isin='BRALSOACNOR5',ex_date='2023-05-02',
    ...
    status='SOURCE_SCHEDULE_REVIEWED_REPLACE_RAW_GROUP_WITH_TWO_EXPLICIT_INSTALLMENTS',
