...
root=Path.cwd();raw=Path('C:/Users/Superleo13/stocks-predictor-work/data');dest=root/'work/h17-source-identity';dest.mkdir(exist_ok=True)
def extract(year):
 ...
 with zipfile.ZipFile(p) as z:
  for name in z.namelist():
   with z.open(name) as stream:
    for row in stream:
     if row[:2]!=b'01' or row[24:27]!=b'010':continue
     ...
     r=records.setdefault(key,{'ticker':tk,'isin':isin,'distribution':dist,'spec':spec,'bdi':bdi,'first_date':date,'last_date':date,'sessions':0})
