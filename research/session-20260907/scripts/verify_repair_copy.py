...
report={'all_historical_tables_unchanged':all(r['unchanged'] for r in checks),'checks':checks,
        'new_counts':{name:dst.execute(f'SELECT COUNT(*) FROM {name}').fetchone()[0]
                      for name in ('fundamentals_pit','shares_pit','cash_events','cash_event_coverage','ingestion_issues')}}
src.close()
dst.close()
(ROOT/'outputs/repair-historical-integrity.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
