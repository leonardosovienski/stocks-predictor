...
| Achados aceitos pelo dono atualizados na qualificação | D-21 | ST-F007/F008 seguiam `OPEN`: corrigido na PR #48 do `predictor-qualification` [execução local] |
...
|---|---|---|
| `v2/policy.py` | `STATUSES` validados; só `APPROVED` decide; `decision_if_approved` registra o hipotético | `test_unapproved_policy_never_decides_but_records_what_it_would_decide` |
...
| `evidence/ledger/stocks-domain-ledger-index.json` | Índice verificável do ledger de domínio (56 registros, cabeça `f9dc9367…`), sem dados de processo | sha256 do ledger igual antes e depois; zero ocorrências de hostname, `process`, `pid` e usuário |
| `evidence/review-20260925/` | Recibos R8 novos do pacote alterado | `verify_operational_evidence` |
| `predictor-qualification` PR #48 | ST-F007/F008 `ACCEPTED_BY_OWNER` (D-21); GATES com nota; attestation reemitida (P2 abertos 5 → 3); a anterior foi preservada como `_superseded_` | `attest.py check` e `verify_attestation` (`all_ok`) locais |
...
- `pytest tests/test_v2_*.py`: **125 passed** [execução].
...
- `predictor-qualification`: `attest.py check` e `verify_attestation` da PR #48 com `all_ok: true` [execução local].
...
A suíte completa isolada, o R8 e o CI estão na seção 10, preenchida depois do commit, porque a suíte exige árvore
limpa.
...
## 10. Suíte isolada, R8 e CI

Preenchido depois do commit das correções.
...
- **Decisões do dono:**
  ...
  - fazer o merge da PR #48 do `predictor-qualification` e da PR desta revisão;
